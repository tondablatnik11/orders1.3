...
const PickingDetails = ({ details }) => {
    if (!details || details.length === 0) {
        return null; // Pokud nejsou data, nic nezobrazíme
    }
    return (
...
const OrderDetailsModal = ({ order, onClose, onShowHistory }) => {
    ...
    const orderEntries = Object.entries(order).filter(([key]) => !['picking_details', 'id', 'created_at', 'updated_at', 'Note'].includes(key));
        ...
        <Modal isOpen={!!order} onClose={onClose} title={`Detail zakázky: ${order['Delivery No']}`}>
            <div className="p-0">
                ...
                <div className="px-6 mt-4">
                    ...
                    <textarea value={note} onChange={(e) => setNote(e.target.value)} className="w-full p-2 border rounded-md" rows="3"></textarea>
                </div>
