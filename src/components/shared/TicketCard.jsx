        ...
        <Draggable draggableId={ticket.id} index={index}>
            {(provided, snapshot) => (
                <div
                    ...
                    onClick={onClick}
                    className={`bg-slate-700 p-3 rounded-lg mb-2 shadow-md hover:bg-slate-600 transition-colors cursor-pointer border-l-4 ${snapshot.isDragging ? 'border-blue-500' : 'border-slate-600'}`}
                >
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-600/50">
                        ...
                        <div className="flex items-center gap-3 text-slate-400">
                           ...
                           {ticket.commentCount > 0 && <div className="flex items-center gap-1"><MessageSquare className="w-4 h-4" /><span className="text-xs">{ticket.commentCount}</span></div>}
                        </div>
                    </div>
                </div>
            )}
