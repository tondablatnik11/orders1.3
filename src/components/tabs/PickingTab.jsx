...
import { Package, Weight, UploadCloud, ChevronDown, ChevronUp, ArrowUpDown, Clock, UserCheck, Users, Calendar, X, BarChart2, TrendingUp, ChevronsRight } from 'lucide-react';
import { format, startOfDay, endOfDay, parseISO, isWithinInterval, startOfWeek, endOfWeek, eachDayOfInterval, getWeek, eachWeekOfInterval, startOfMonth, endOfMonth, eachMonthOfInterval } from 'date-fns';
...
const PickingTab = () => {
    ...
    const shiftChartData = useMemo(() => {
        ...
        return interval.map(date => {
            ...
            opsInInterval.forEach(op => {
                ...
                if (isMorningShift) {
                    if (isEvenWeek) point['Směna A']++; else point['Směna B']++;
                } else if (isAfternoonShift) {
                    if (isEvenWeek) point['Směna B']++; else point['Směna A']++;
                }
            });
            return point;
        });
    ...
    const sortedFilteredData = useMemo(() => {
        let sortableItems = [...filteredDataByDate].filter(row => 
            Object.values(row).some(value => String(value).toLowerCase().includes(filters.global.toLowerCase()))
        );
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
                            ...
                            <ResponsiveContainer width="100%" height={300}>
                                <BarChart data={shiftChartData}>
                                    ...
                                    <Legend />
                                    <Bar dataKey="Směna A" fill="#f59e0b" stackId="a" />
                                    <Bar dataKey="Směna B" fill="#6366f1" stackId="a" />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                        ...
                        <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
                            <div className="p-6">
                                ...
                                <div className="mt-4"><input type="text" placeholder="Hledat v detailech..." value={filters.global} onChange={(e) => { setFilters({global: e.target.value}); setCurrentPage(1); }} className="p-2 bg-slate-700 border border-slate-600 rounded-md w-full md:w-1/3 text-white placeholder-slate-400" /></div>
                            </div>
